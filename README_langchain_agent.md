...
系统按“入口层、工作流编排层、领域服务层、工具与知识层、持久化层、候选执行沙箱”拆分。主链路由 `AutoSolverLangChainAgent` 负责装配运行配置和依赖，再交给 `AutoSolverWorkflow` 驱动 LangGraph 节点或顺序 fallback。

```mermaid
flowchart TB
  CLI["CLI / Python API"] --> Agent["AutoSolverLangChainAgent"]
  Agent --> CaseIO["caseio: TSV 读取、诊断、特征与评分基础函数"]
  Agent --> Workflow["AutoSolverWorkflow"]
  Workflow --> Classifier["InstanceClassifier"]
  Workflow --> Planner["PlannerToolbox + StrategyLibrary + SolverSkillLibrary"]
...
2. `caseio` 加载 TSV case，宽松模式会保留可解析行并输出诊断，`--strict-cases` 会在坏行处失败。
3. `InstanceClassifier` 汇总实例规模、任务组比例、意愿分布、容量等特征。
4. `PlannerToolbox` 向 LLM 暴露只读工具：实例特征、策略库、相似历史、UCB 推荐、当前最佳 artifact 摘要。
5. `LLMCodeGenerator` 先生成 `SolverPlan`，再生成 `CandidateEnvelope`，并用 Pydantic 校验结构化输出。
6. `Validator` 先做 AST 安全检查，再用 `runtime.run_candidate` 在子进程中执行 smoke case。
7. `Scorer` 对有效候选做多 case 评分，排序 rank 为 `(failures, -total_covered, total_penalty, total_runtime)`。
8. `MemoryStore` 写入短期运行记忆、长期实验记录、相似检索特征和 UCB bandit 统计。
9. `ArtifactStore` 和 `EventRecorder` 写出候选代码、验证结果、评分结果、影响分析、事件日志与最终报告。
10. `finalize` 对 top-k 候选按正式超时复评，输出最终 solver 与 JSON report。

## 目录结构

```text
.
├── langchain_autosolver_agent.py        # 兼容 CLI 入口，委托 autosolver_agent.cli
├── README.md                            # 简短入口文档
├── README_langchain_agent.md            # 完整说明文档
├── RELEASE.md                           # v1.0.0 发布说明
├── Dockerfile                           # Docker 运行时镜像
├── pyproject.toml                       # 包元数据、依赖、工具配置
├── requirements.txt                     # 运行依赖兼容清单
├── autosolver_agent/
...
| `autosolver_agent.workflow.graph` | 定义 `classify -> generate -> validate_and_score -> finalize` 工作流，支持 LangGraph 和顺序 fallback。 |
| `autosolver_agent.workflow.services` | 封装生成、评估、修复、收尾、报告构建等服务边界。 |
| `autosolver_agent.llm` | 负责 `SolverPlan`、`CandidateEnvelope` 的结构化生成、解析、schema 修复和验证修复。 |
| `autosolver_agent.tools` | 提供实例分类、LangChain planner tools、安全验证和候选评分。 |
...
| `autosolver_agent.skills` | 内置策略库、solver 技能库和参考 solver 摘要，作为 LLM 提示上下文。 |
...
```bash
.venv/bin/python langchain_autosolver_agent.py --cases examples/demo_case.txt --iterations 3
```
...
- Docker 兼容标签：`autosolver-agent:latest`
