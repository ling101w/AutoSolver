"""Standalone solver template rendered by ``langchain_autosolver_agent``.

The string ``SOLVER_TEMPLATE`` here contains the **runtime** code that is
written to ``generated_submit_solution.py``. It deliberately uses only the
Python standard library so the produced file can be loaded by the judge
without any LangChain dependency.

The placeholder ``__CONFIG__`` is substituted with a Python literal dict at
render time.
"""
